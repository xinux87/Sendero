...
NO_VALUE = -2000000
ALTITUDE_MIN_CM = -1103400  # Mariana Trench depth — anything below is an invalid GPS reading


def _parse_altitude(raw: str) -> int:
    """Parse one altitude entry from correct_altitude or altitude fields.

    Some devices/firmware use a plain integer ("255241"), others a
    comma-separated pair ("3,-20000") where the last value is the altitude.
    Values below the Mariana Trench depth are physically impossible and are
    replaced with NO_VALUE so the interpolation forward-fills them.
    """
    val = int(raw.split(",")[-1])
