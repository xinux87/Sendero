...
from core.mifit.api import WorkoutDetailData, WorkoutSummary

NO_VALUE = -2000000
ALTITUDE_MIN_CM = -1103400  # Fosa de las Marianas — por debajo es lectura GPS inválida


def _parse_altitude(raw: str) -> int:
    """Parsea una entrada de altitud de correct_altitude o altitude.

    Algunos dispositivos usan un entero plano ("255241"), otros un par separado
    por coma ("3,-20000") donde el último valor es la altitud. Valores por
    debajo de la Fosa de las Marianas son físicamente imposibles y se sustituyen
    por NO_VALUE para que la interpolación los rellene hacia delante.
    """
    val = int(raw.split(",")[-1])
    return val if val >= ALTITUDE_MIN_CM else NO_VALUE
...
def parse_track_data(summary: WorkoutSummary, detail: WorkoutDetailData):
    return RawTrackData(
        ...
        alt=array.array(
            "q",
            [_parse_altitude(val)
             for val in list(filter(None, (detail.correct_altitude or detail.altitude).split(";")))]
            if (detail.correct_altitude or detail.altitude)
            else [],
        ),
        hrtimes=array.array(
            "q",
